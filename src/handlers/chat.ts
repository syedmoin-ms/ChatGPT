...
class ChatHandler {
  ...
  handle = async (msg: TelegramBot.Message, text: string) => {
    ...
    const reply = await this._bot.sendMessage(
      chatId,
      this._opts.queue ? 'by Syed Moinuddin' : 'Typing...',
      {
    ...
    if (!this._opts.queue) {
      ...
      await this._bot.editMessageText(
        this._n_queued > 0 ? `by Syed Moinuddin: You are #${this._n_queued} in line.` : 'Typing...',
        {
          chat_id: chatId,
      ...
      this._updatePositionQueue.add(() => {
        return this._bot.editMessageText(
          this._positionInQueue[key] > 0
            ? `by Syed Moinuddin: You are #${this._positionInQueue[key]} in line.`
            : 'Typing...',
          {
            chat_id,
